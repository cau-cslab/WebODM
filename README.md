...
```bash
# 이 스크립트를 한 흐름에 실행해야 함. 껐다 켜면 또 안됨... 왜인지는 모름
...
# dev -> 이걸로 해야 개발한게 보임
sudo ./webodm.sh start --dev

# 기본 with GPU
sudo ./webodm.sh start --gpu

# dev with GPU -> 이걸로 해야 개발한게 보임
sudo ./webodm.sh start --dev --gpu
```

# 이하는 기존 WebODM README
